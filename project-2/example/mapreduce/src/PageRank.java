    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i < 2*numRepititions; i++) {
            ...
            if(i%2 == 0) {
            	...
            	Counter loss = job.getCounters().findCounter(COUNTERS.LOSS_MASS_COUNT);
            	leftover = loss.getValue();
            	Counter nodeNum = job.getCounters().findCounter(COUNTERS.SIZE);
            	size = nodeNum.getValue();
System.out.println("======>After Stage" + i + " leftover: " + leftover + " size: " + size);
            } else {
                // Set up leftover and size
            	leftover = 0;
            	size = 0;
